...
export class JiiStore<T> extends CustomStore {
  ...
  constructor(
    ...
    super({
      ...
      byKey: (key) => {
        ...
        return http.get<T>(this.byKeyUrl(environment, options, basePath, key)).toPromise();
      },
      load: (loadOptions: LoadOptions) => {
        if (options.loadByGet) return http.get<Page<T>>(this.loadUrl(environment, loadOptions, options, basePath))
          .toPromise()
          .then((page: Page<T>) => {
        ...
        return http.post<JiiPage<T>>(this.loadUrl(environment, loadOptions, options, basePath), this.payloadFromLoadOptions(loadOptions))
          .toPromise()
   ...
   * @param key
   */
  private byKeyUrl(environment: any, options: JiiStoreOptions, basePath: string, key: any): string {
   ...
   * @param basePath 
   */
  private loadUrl(environment: any, loadOptions: LoadOptions, options: JiiStoreOptions, basePath: string): string {
